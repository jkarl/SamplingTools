 ...
 return(nameme[[1]][1])
  ...
  # IF shnystr exists already, DON"T DO THIS
   ...
   #     if(is.null(theSHP$shnystr)) {
    ...
    #}
        ...
        # Diagnostic save
        writeOGR(theSHP, theSHP$dirname[[1]], "stratatest", driver = "ESRI Shapefile", overwrite_layer = TRUE)
   ...
   theBaseName()
      ...
      #  })
        ...
        dat$ID <- ids
        theSHP@data <- dat
        save(theSHP, file = paste(theSHP$dirname[[1]], "shapedata", sep="//"))
        writeOGR(theSHP, theSHP$dirname[[1]], "rgdaltest2", driver = "ESRI Shapefile", overwrite_layer = TRUE)
        test.attframe <- read.dbf(paste(theSHP$dirname[[1]], "rgdaltest2", sep="//"))
        grtsstrata <- "shnystr"
        ...
        save(result, file = paste(theSHP$dirname[[1]], "results", sep="//"))
        writeOGR(result, theSHP$dirname[[1]], "results", driver = "ESRI Shapefile", overwrite_layer = TRUE)
        print("DONE creating sample locations!")
        file.copy(paste(as.character(theSHP$dirname[[1]]),"//", "rgdaltest2.prj", sep=""), 
                  paste(as.character(theSHP$dirname[[1]]),"//", "results.prj", sep=""), overwrite = TRUE)
      
      # was necessary in CentOS in previous server.R file
      system("cp -f ./rgdaltest2.prj ./results")
      
      # have to make latlong version here
        theResults <- readOGR(theSHP$dirname[[1]], "results")
        projit <- spTransform(theResults, CRS("+proj=longlat +ellps=WGS84"))
        writeOGR(projit,theSHP$dirname[[1]] , paste(theBaseName(),"latlong", sep=""), 
      ...
      print(getwd())
        switch(Sys.info()[["sysname"]], Windows = {
          ...
          system(sprintf("zip latlong %s %s %s", typesll_as_string))
        })
      })
      x<- 0
      enable(updateNumericInput(session, "pts", value = x))
  ...
  #   })
    ...
    grtsUNEQPointData <- reactive({
      ...
      save(resultuneq, file = paste(theSHP$dirname[[1]], "results", sep="//"))
      writeOGR(resultuneq, theSHP$dirname[[1]], "results", driver = "ESRI Shapefile", overwrite_layer = TRUE)
      print("DONE creating sample locations!")
      file.copy(paste(as.character(theSHP$dirname[[1]]),"//", "rgdaltest2.prj", sep=""), 
                paste(as.character(theSHP$dirname[[1]]),"//", "results.prj", sep=""), overwrite = TRUE)
      ...
      theResults <- readOGR(theSHP$dirname[[1]], "results")
      projit <- spTransform(theResults, CRS("+proj=longlat +ellps=WGS84"))
      writeOGR(projit,theSHP$dirname[[1]] , "latlong", driver = "ESRI Shapefile", overwrite_layer = TRUE)
      setwd(theSHP$dirname[[1]])
      # switch tab panels
      updateTabsetPanel(session, "inTabSet", selected = "Plot")
      # create zip fle in case user wants the points
