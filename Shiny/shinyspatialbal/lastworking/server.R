...
#   save(blah, file="testme.dat")    
  ...
  # alter shapefile fields for stratum
    mySHPdata<-reactive({
      ...
      #Diagnostic  save
      writeOGR(theSHP, "/srv/shiny-server/shinyspatialbal/", "stratatest", driver="ESRI Shapefile", overwrite_layer=TRUE, verbose=TRUE)
  ...
  # make our sample design and invoke grts from spsurvey
    grtsPointData <- reactive({
     ...
     isolate({
      ...
      writeOGR(theSHP, "/srv/shiny-server/shinyspatialbal/", "rgdaltest2", driver="ESRI Shapefile", overwrite_layer=TRUE, verbose=TRUE)
    ...
    # try this bit of nonsense
      ...
      result <- grts2(design=as.list(wholedesign), src.frame="sp.object", sp.object=theSHP, type.frame=myTypeFrame,
                   stratum=grtsstrata,
    ...
    file.copy("/srv/shiny-server/shinyspatialbal/rgdaltest2.prj", "/srv/shiny-server/shinyspatialbal//grtsresults.prj", 
          overwrite=TRUE, copy.mode=TRUE)

     # no idea why he above doesn't work in CentOS; brute force method
        system("cp -f ./srv/shiny-server/shinyspatialbal/rgdaltest2.prj /srv/shiny-server/shinyspatialbal/grtsresults.prj")
#       textOutput("should be copying")
    
     save(result, file="result")
     writeOGR(result, "/srv/shiny-server/shinyspatialbal/", "grtsresults", driver="ESRI Shapefile", overwrite_layer=TRUE, verbose=TRUE)
     print("DONE creating sample locations!")
      
      # have to make latlong version here
      theResults <- readOGR(".", "grtsresults")
      projit<-spTransform(theResults, CRS("+proj=longlat"))
      writeOGR(projit, ".", "latlongresults", driver="ESRI Shapefile", overwrite_layer=TRUE)          
      ...
      if (length(latlongtypes) > 1) for (j in 2:length(latlongtypes))
        latlong_types_as_string = paste(latlong_types_as_string," ",as.character(latlongtypes[[j]]),sep="")
  ...
  output$pointdata <- renderTable({
   ...
   output$downloadLatlong <- downloadHandler(
     filename = function() {
